...
export async function labAnalysisAgent(state: typeof AgentState.State) {
    ...
    if (!extractedText && filePath) {
        try {
            ...
            console.log(`[Lab Analysis] Extracted ${extractedText.length} characters from PDF`);
        } catch (error) {
            console.error("Failed to extract PDF:", error);
            return {
                messages: [{
                    role: "assistant",
                    content: "I encountered an error reading the PDF file. Please try uploading it again."
                }]
            };
        }
    }
    ...
    if (toolAlreadyCalled) {
        console.log("[Lab Analysis] Tool already executed. Adding stop instruction to system prompt.");
    ...
    // Build messages - CRITICAL FIX: Filter and convert messages to Gemini-compatible format
    let messagesWithSystem;
    if (messages.length === 0) {
        // First run - just system + PDF content
        messagesWithSystem = [
        ...
        // Follow-up run - convert existing messages to Gemini-compatible format
        ...
        console.log(`[Lab Analysis] Follow-up run with ${compatibleMessages.length} converted messages`);
    }

    try {
        console.log(`[Lab Analysis] Invoking model with ${messagesWithSystem.length} messages`);
        console.log(`[Lab Analysis] System prompt length: ${systemPrompt.length}`);

        // Debug: Log message types
        console.log(`[Lab Analysis] Message types: ${messagesWithSystem.map((m: any) => m._getType()).join(', ')}`);
        ...
        console.log(`[Lab Analysis] Response generated with ${toolCallsCount} tool calls`);
        ...
        if (response.tool_calls) {
            for (const toolCall of response.tool_calls) {
                if (toolCall.name === "save_lab_results" && toolCall.args.results) {
                    ...
                    console.log(`[Lab Analysis] Extracted ${labResults.length} lab results for state`);
                }
            }
        }
        ...
        if (error.message?.includes("ECONNREFUSED") || error.message?.includes("fetch failed")) {
            return {
                messages: [{
                    role: "assistant",
                    content: "Cannot connect to model server. Please ensure it is running."
                }]
            };
        }

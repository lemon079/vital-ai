...
export async function conversationAgent(state: typeof AgentState.State) {
    ...
    if (labResults && labResults.length > 0) {
        console.log(`[Conversation] Adding ${labResults.length} lab results to context`);
...
${summary}`;
    ...
    console.log(`[Conversation] System prompt length: ${systemMsgContent.length} characters`);
    console.log(`[Conversation] Has lab results: ${!!(labResults && labResults.length > 0)}`);
